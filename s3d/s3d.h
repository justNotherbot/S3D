...
namespace s3d
{
    ...
    private:
        ...
        //}
    public:
        Renderer(short a, short b) //constructor
        {
            width = a;
            height = b;
        }
        color* createScreenBuffer(color fill_color = {0, 0, 0})
        {
            screen_buffer = new color[width * height] {fill_color};
        ...
        void fillCell(short x, short y, color c = {0, 0, 0}) //Fills a cell with some symbol. By default it's a fill symbol
        {
            screen_buffer[x + y * width] = c;
        }
        void fillScreenBuffer(color fill_color = { 0, 0, 0 })
        {
            for (int i = 0; i < width * height; i++)
            {
                screen_buffer[i % width + int(i / width)] = fill_color;
            }
        }
        void clear_z_buffer()
        {
        ...
        void drawLine(double x, double y, double x1, double y1, color c = {0, 0, 0}) //Draws line on console. Uses linear interpolation.
        {
            double a, tmp_y, f;
            if (x != x1)
            {
                ...
                {
                    tmp_y = a * n + b;
                    fillCell(std::round(n), std::round(tmp_y), c);
                }
            }
            else
            {
        ...
        void drawNgon(short *coord[], int n, color c = { 0, 0, 0 }) //Connects points of ngon in the same order as they are specified
        {
            for (int i = 0; i < n+2; i += 2)
            {
                if (*coord[i] >= 0 && *coord[i + 1] >= 0 && *coord[i + 2] >= 0 && *coord[i + 3] >= 0)
                    drawLine(*coord[i], *coord[i + 1], *coord[i + 2], *coord[i + 3], c);
                else
                    break;
            }
            drawLine(*coord[0], *coord[1], *coord[n * 2 - 2], *coord[n * 2 - 1], c);
        }
        void fillTri(short coordinates[], color c = { 0, 0, 0 })
        {
            short* tmp[6];
            ...
            {
                drawLine(*tmp[0] + std::round(decrement_x * (i - *tmp[1])), i, std::round(*tmp[0] + increment_x * (i - *tmp[1])), i, c);
            }
            //Drawing the bottom part
            increment_x = double(*tmp[4] - *tmp[2]) / double(*tmp[5] - *tmp[3]);
            for (int i = *tmp[3] + 1; i <= *tmp[5]; i++)
            {
                drawLine(*tmp[0] + std::round(decrement_x * (i - *tmp[1])), i, std::round(*tmp[2] + increment_x * (i - *tmp[3])), i, c);
            }
        }
        void drawCircle(int h, int v, int r, color c) //Draws circle. It actually looks more like an oval because of console cells' width/hight ratio
        {
            double y;
            for (double x = (double)h - (double)r; x <= (double)h + (double)r; x += 0.1) //Using the equation of a circle on coordinate space here
            {
                ...
                fillCell((short)x, (short)y, c);
                fillCell((short)x, (short)v - ((short)y - v), c);
            }
            fillCell((short)h + r - 1, (short)v, c);
        }
        ...
        void drawTri3d(double fov, const double cam_coords[], triangle tri, color c = {0, 0, 0}) //Project 3d triangle onto the screen surface
        {
            short tmp[6] = {0};
            ...
            if (draw) //Drawing triangle
            {
                ...
                //drawLine(short(tmp[0]), short(tmp[1]), short(tmp[2]), short(tmp[3]), ch, 5);
                //drawLine(short(tmp[2]), short(tmp[3]), short(tmp[4]), short(tmp[5]), ch, 5);
                //drawLine(short(tmp[4]), short(tmp[5]), short(tmp[0]), short(tmp[1]), ch, 5);
            }
        }
